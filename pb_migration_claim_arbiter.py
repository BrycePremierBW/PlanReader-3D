...
def arbitrate_claims(
    *,
    ...
    for key, rows in new_groups.items():
        ...
        accepted = [item for item in rows if not item.abstained and item.value is not None]
        if len(accepted) > 1:
    ...
    blocking = any(
        item.kind in {"duplicate_new_claim", "conflicting_new_claims", "semantic_key_collision"}
