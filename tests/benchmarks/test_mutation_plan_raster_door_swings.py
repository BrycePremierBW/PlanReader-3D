"""Inverted-CAD door-swing symbols emit unlabeled D2 interiors and D1 exteriors."""
...
def _double_leaf(image: np.ndarray, left_x: int, hinge_y: int, radius: int = 36, inner: int = 30) -> None:
    def ring(cx: int, cy: int, start: int, end: int) -> None:
        outer_pts = []
        inner_pts = []
        for angle in range(start, end + 1):
            rad = math.radians(angle)
            outer_pts.append(
                [int(cx + radius * math.cos(rad)), int(cy + radius * math.sin(rad))]
            )
            inner_pts.append(
                [int(cx + inner * math.cos(rad)), int(cy + inner * math.sin(rad))]
            )
    ...
    ring(left_x, hinge_y, 270, 360)
    ring(left_x + 2 * radius, hinge_y, 180, 270)


def _dark_double_leafs(count: int) -> np.ndarray:
    image = np.zeros((280, 160 + count * 200, 3), np.uint8)
    for i in range(count):
        _double_leaf(image, 50 + i * 200, 180)
    return image
...
def test_existing_d1_blocks_exterior_but_d2_does_not() -> None:
    assert should_emit_exterior_door_total(2, [])
    assert should_emit_exterior_door_total(2, ["D2", "W1"])
    assert not should_emit_exterior_door_total(2, ["D1"])
    assert not should_emit_exterior_door_total(2, ["D-01"])
    assert not should_emit_exterior_door_total(1, [])
    assert not should_emit_exterior_door_total(5, [])
...
def test_light_background_raster_is_ignored() -> None:
    ...
    _double_leaf(light, 80, 260)
    _double_leaf(light, 260, 260)
    assert interior_door_swings_from_raster(light) == []
    assert exterior_double_leaf_swings_from_raster(light) == []
    ...
    d1_preds = {
        ...
        for p in GenericPlanReaderExtractor().extract_from_pdf(
            _pdf_with_raster(
                tmp_path,
                "sched-d1.pdf",
                _dark_double_leafs(2),
                title="WINDOW AND DOOR SCHEDULE  SCALE 1:50",
            )
        )
    }
    assert "D1" not in d1_preds


def test_two_double_leaf_swings_mutate_d1(tmp_path: Path) -> None:
    two = {
        p.tag: p
        for p in GenericPlanReaderExtractor().extract_from_pdf(
            _pdf_with_raster(tmp_path, "d1-two.pdf", _dark_double_leafs(2))
        )
    }
    three = {
        p.tag: p
        for p in GenericPlanReaderExtractor().extract_from_pdf(
            _pdf_with_raster(tmp_path, "d1-three.pdf", _dark_double_leafs(3))
        )
    }
    assert two["D1"].quantity == 2.0
    assert three["D1"].quantity == 3.0
    assert two["D1"].trade_type == "doors"
    assert "D2" not in two
    assert "D2" not in three
    assert "W1" not in three
    assert exterior_double_leaf_swings_from_raster(_dark_double_leafs(2))
    assert interior_door_swings_from_raster(_dark_double_leafs(2)) == []


def test_existing_d1_card_keeps_authority_over_exterior_swings(tmp_path: Path) -> None:
    path = tmp_path / "card-d1.pdf"
    rgb = _dark_double_leafs(2)
    raster = tmp_path / "card-d1.png"
    Image.fromarray(rgb).save(raster)
    doc = fitz.open()
    ...
    page.insert_image(
        fitz.Rect(40, 280, 40 + rgb.shape[1], 280 + rgb.shape[0]),
        filename=str(raster),
    )
    doc.save(path)
    doc.close()
...
def test_interior_d2_does_not_block_exterior_d1(tmp_path: Path) -> None:
    rgb = np.zeros((420, 820, 3), np.uint8)
    _quarter_ring(rgb, 90, 190, 46, 40)
    _quarter_ring(rgb, 220, 190, 46, 40)
    _quarter_ring(rgb, 350, 190, 46, 40)
    _double_leaf(rgb, 50, 360)
    _double_leaf(rgb, 250, 360)
    preds = {
        p.tag: p
        for p in GenericPlanReaderExtractor().extract_from_pdf(
            _pdf_with_raster(tmp_path, "both.pdf", rgb)
        )
    }
    assert preds["D2"].quantity == 3.0
